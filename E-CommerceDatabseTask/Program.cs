    ...
    public class Program
    {
        ...
        {
            {
                if (loggedInUserId != 0)
                {
                    ...
                    foreach (var order in userOrders)
                    {
                        ...
                        {
                            Console.WriteLine($"  - {item.PID.ProductName} (Quantity: {item.Quantity})");
                        }
                    }
                }
                else
        ...
        static void ViewOrderDetails()
        {
            ...
            if (order != null)
            {
                ...
                foreach (var item in order.OrderProducts)
                {
                    double itemTotal = item.PID.Price * item.Quantity;
                    orderTotal += itemTotal;
                    Console.WriteLine($"  - {item.PID.ProductName} (Quantity: {item.Quantity}) - Price: {item.PID.Price} each");
                }
        ...
        static void AddReview()
        {
            if (loggedInUserId != 0)
            {
                ...
                Review review = context.Review.FirstOrDefault(r => r.OrderId == id);
        ...
        static void ViewReviewsForProduct()
        {
            ...
            if (product != null)
            {
                var productOrders = context.OrderProducts
                    .Include(po => po.OID)
                ...
                if (productOrders.Count != 0)
                {
                    foreach (var item in productOrders)
                    {
                        ...
                        if (item.OID.Review != null)
                        {
                            Console.WriteLine($"Rating: {item.OID.Review.Rating}");
                            Console.WriteLine($"Comment: {item.OID.Review.Comment}");
                        }
                        else
                        {
